...
use AppGallery\kits\Kit;
use AppGallery\Main;
use AppGallery\task\ArenaTask;
use AppGallery\utils\BackupUtils;
use AppGallery\utils\BlockUtils;
...
class Arena extends MainExtension implements Listener{
    ...
    /** @var int */
    public int $status = ArenaStatus::DISABLED;
    /** @var BackupUtils */
    ...
    public function __construct(array $data = []){
        ...
        $this->backupUtils = new BackupUtils($this->getMain(), $this);
        ...
        $this->setStatus($this->config->get("status", ArenaStatus::DISABLED));
        $this->getScheduler()->scheduleRepeatingTask(new ArenaTask($this), 20);
        $this->getServer()->getPluginManager()->registerEvents($this, $this->getMain());
        $this->loadSpawns();
    }

    public function loadSpawns(): void{
    ...
    public function activate(): void{
        $this->config->set("status", ArenaStatus::WAITING);
        $this->config->save();
    ...
    public function forceStart(): void{
        ...
        $this->setStatus(ArenaStatus::RUNNING);
        $this->runningTime = 1;
    }

    public function disable(): void{
        $this->config->set("status", ArenaStatus::DISABLED);
        $this->config->save();
        $this->setStatus(ArenaStatus::DISABLED);
    }
    ...
    public function isPlaying($player): bool{
        ...
        return in_array($player, $this->cache_slots);
    }
    ...
    public function onBlockBreakEvent(BlockBreakEvent $event): void{
        $player = $event->getPlayer();
        if ($this->isPlaying($player)){
            if($this->getStatus() === ArenaStatus::WAITING || $this->getStatus() === ArenaStatus::RESETTING){
                $event->cancel();
            }
        }
    }
    ...
    public function onDamage(EntityDamageEvent $event): void{
        $player = $event->getEntity();
        if ($player instanceof Player && $this->isPlaying($player) && $this->getStatus() !== ArenaStatus::RUNNING) {
            $event->cancel();
